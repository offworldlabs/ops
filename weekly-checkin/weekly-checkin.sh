#!/bin/bash
# Post a fixed weekly check-in nudge to a ClickUp chat channel.
#
...
# /root/weekly-checkin/weekly-checkin.env, or set CHECKIN_CONFIG). No
# credentials or site-specific IDs live in this file, so it is safe to keep in a
# public repository. See weekly-checkin.env.example.
#
# Usage:
#   ./weekly-checkin.sh                  # respects the scheduled-hour guard
#   SKIP_TIME_GUARD=1 ./weekly-checkin.sh    # post now, ignoring the guard
#   DRY_RUN=1 SKIP_TIME_GUARD=1 ./weekly-checkin.sh   # show the request, post nothing

set -euo pipefail

CONFIG_FILE="${CHECKIN_CONFIG:-/root/weekly-checkin/weekly-checkin.env}"
...
_overrides="$(declare -p CLICKUP_WORKSPACE_ID CLICKUP_CHANNEL_ID CHECKIN_MESSAGE \
    CHECKIN_TZ CHECKIN_HOUR CLICKUP_TOKEN_FILE CHECKIN_LOG 2>/dev/null || true)"
...
CLICKUP_TOKEN_FILE="${CLICKUP_TOKEN_FILE:-/root/weekly-checkin/.clickup_token}"
CHECKIN_LOG="${CHECKIN_LOG:-/root/weekly-checkin/log.jsonl}"
CHECKIN_TZ="${CHECKIN_TZ:-Europe/London}"
CHECKIN_HOUR="${CHECKIN_HOUR:-09}"
...
fail() {
    local msg="$1"
    echo "weekly-checkin FAILED: ${msg}" >&2
    logger -t weekly-checkin -p user.err "FAILED: ${msg}" 2>/dev/null || true
    printf '{"ts":"%s","ok":false,"error":%s}\n' \
        "$(date -Is)" "$(printf '%s' "${msg}" | jq -Rs .)" >> "${CHECKIN_LOG}" 2>/dev/null || true
    exit 1
}
...
[ -n "${CHECKIN_MESSAGE:-}" ]      || fail "CHECKIN_MESSAGE is not set (config: ${CONFIG_FILE})"

# Scheduled-hour guard. Cron cannot express a timezone on Debian/Ubuntu (no
# CRON_TZ), so when the host clock is not in CHECKIN_TZ the schedule must fire
...
if [ "${SKIP_TIME_GUARD:-0}" != "1" ] && [ "$(TZ="${CHECKIN_TZ}" date +%H)" != "${CHECKIN_HOUR}" ]; then
    exit 0
fi
...
BODY="$(jq -nc --arg c "${CHECKIN_MESSAGE}" \
    '{type:"message",content:$c,content_format:"text/plain"}')"

if [ "${DRY_RUN:-0}" = "1" ]; then
    echo "weekly-checkin DRY RUN — nothing sent"
    echo "  POST ${URL}"
    echo "  body ${BODY}"
...
printf '{"ts":"%s","ok":true,"http":%s,"message_id":"%s","channel":"%s"}\n' \
    "$(date -Is)" "${HTTP_CODE}" "${MSG_ID}" "${CLICKUP_CHANNEL_ID}" >> "${CHECKIN_LOG}"

echo "weekly-checkin OK: posted message ${MSG_ID}"
